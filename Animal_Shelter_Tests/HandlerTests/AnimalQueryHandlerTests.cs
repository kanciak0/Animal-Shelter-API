...
public class AnimalQueryHandlersTests
{
    ...
    public async Task GetAnimalsQueryHandler_ReturnsAnimals()
    {
        ...
        Assert.AreEqual(animals[0].Species.Breed, result[0].Species.Breed);
    }
    [TestCleanup]
    [TestMethod]
    public async Task GetSpecificAnimalQueryHandler_ReturnsAnimal()
    {
        ...
        _mockRepository.Setup(r => r.GetByID(animalId)).Returns(animal);
        ...
        Assert.AreEqual(animal.Species.Breed, result.Species.Breed);
    }
}
